...
******** Self-reported Disease History **********
	if inlist(wave,98){
		foreach k in a b c d e f g h i j k l m {
			gen disease_`k' = 0 if g17`k'1==2 // no
			replace disease_`k' = 1  if g17`k'2 == 3 // yes but no disability
			replace disease_`k' = 2  if g17`k'2 == 2	// yes but more or less  disability 
			replace disease_`k' = 3  if g17`k'2 == 1
		}
		...
		label define disease 0"no"   1"yes,but no disability" 2"yes, cause more or less disability" 3"yes, cause serious disability"
	
	if inlist(wave,00,02,05,08,11,14,18){
		foreach k in a b c d e f g h i j k l m n o {
			gen disease_`k' = 0 if g15`k'1==2 // no
			replace disease_`k' = 1  if g15`k'3 == 3 // yes but no disability
			replace disease_`k' = 2  if g15`k'3 == 2	// yes but more or less  disability 
			replace disease_`k' = 3  if g15`k'3 == 1
	...
	if inlist(wave,98,00){
		gen Diahypertension_b`year' 	= .
		gen Diadiabetes_b`year' 	= .
		gen Diaheartdisea_b`year' 	= .
		gen Diastrokecvd_b`year' 	= .
		gen Diacopd_b`year' 	= .
		gen Diatb_b`year' 	= .
		ren disease_g Diacataract_b`year' 	= .
		gen Diaglaucoma_b`year' 	= .
		gen Diacancer_b`year' 	= .
		gen Diaprostatetumor_b`year' 	= .
		gen Diaulcer_b`year' 	= .
		gen Diaparkinson_b`year' 	= .
		gen Diabedsore_b`year' 	= .
		gen Diaarthritis_b`year'  	= .
		gen Diadementia_b`year'	= .
	}		
	
	if inlist(wave,02,05,08,11,14,18){
		foreach k in a b c d e f g h i j k l m n o {
			recode g15`k'2 (5/9 = .),gen(disease_`k')
		}

		ren disease_a Diahypertension_b`year'
		ren disease_b Diadiabetes_b`year'
		ren disease_c Diaheartdisea_b`year'
		ren disease_d Diastrokecvd_b`year'
		ren disease_e Diacopd_b`year'
		ren disease_f Diatb_b`year'
		ren disease_g Diacataract_b`year'
		ren disease_h Diaglaucoma_b`year'
		ren disease_i Diacancer_b`year'
		ren disease_j Diaprostatetumor_b`year'
		ren disease_k Diaulcer_b`year'
		ren disease_l Diaparkinson_b`year'
		ren disease_m Diabedsore_b`year'
		ren disease_n Diaarthritis_b`year' 
		ren disease_o Diadementia_b`year'
	}	
...
******************* cooking fuel **************
	if inlist(wave,11,14,18){	
		gen fuel_b`year'= a537
		gen smell_b`year'= a536
	}	
